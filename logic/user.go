...
// func getRollsForUser(user models.User) int {
// 	var nbrRoll int
// 	err := DB.QueryRow("SELECT availableRoll FROM users WHERE id = ?", user.Id).Scan(&nbrRoll)
// 	if err != nil {
// 		log.Fatal(err)
...
func getTopCollectors(infos IndexInfo) IndexInfo {
	...
	rows, errorm := DB.Raw("SELECT username, count(card_id) FROM users INNER JOIN cards_in_inventory ON cards_in_inventory.user_id = users.id GROUP BY username ORDER BY count(card_id) DESC LIMIT 10 ").Rows()
	if errorm != nil {
		log.Fatal(errorm)
	}
	defer rows.Close()
